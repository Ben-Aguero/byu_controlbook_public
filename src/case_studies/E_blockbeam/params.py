...
u_eq = np.array([m1 * g * z_eq / ell + m2 * g / 2.0])
##### Chapter 5
# Transfer function numerator and denominator
tf_inner_num = [ell / (m2 * ell**2 / 3.0 + m1 * z_eq**2)]
tf_inner_den = [1, 0, 0]
tf_outer_num = [-g]
tf_outer_den = [1, 0, 0]
##### Chapter 6 / 11 # TODO: decide which chapter to put this in
# State space
inertia_eq = m2 * ell**2 / 3.0 + m1 * z_eq**2
A = np.array([[0, 0, 1, 0],
              [0, 0, 0, 1],
              [0, -g, 0, 0],
              [-m1 * g / inertia_eq, 0, 0, 0],])
B = np.array([[0], [0], [0], [ell / inertia_eq]])
